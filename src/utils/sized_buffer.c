...
NODISCARD static bool buffer_eq_impl(const GenericDataConst data1, const size_t size1,
                                     const GenericDataConst data2, const size_t size2) {
	if(size1 != size2) {
		return false;
	}
...
NODISCARD static SizedBuffer sized_buffer_dup_impl(const GenericDataConst data, const size_t size) {
	...
	GenericData new_data = malloc(size);
...
NODISCARD ReadonlyBuffer readonly_buffer_from_sized_buffer(const SizedBuffer buffer) {
	return (ReadonlyBuffer){ .data = (GenericDataConst)buffer.data, .size = buffer.size };
}
