...
NODISCARD int sendDataToConnection(const ConnectionDescriptor* const descriptor, void* toSend,
                                   size_t length);

// sends a string to the connection, makes all write calls under the hood, deals with arbitrary
// large null terminated strings!
NODISCARD int sendStringToConnection(const ConnectionDescriptor* const descriptor, char* toSend);

// just a warpper to send a string buffer to a connection, it also frees the string buffer!
NODISCARD int sendStringBuilderToConnection(const ConnectionDescriptor* const descriptor,
...
NODISCARD int sendMessageToConnection(const ConnectionDescriptor* const descriptor, int status,
